...
/**
 * The script to display and process the configuration form for searchsources of type "sru"
 *
...
$url = new moodle_url('/mod/literature/searchsource/sru/view.php');
$url->param('id', $id);
$PAGE->set_url($url);
...
$form = new literature_searchsource_sru_form();
...
if ($form->is_cancelled()) {
    ...
    $data = $_POST;
    $data['id'] = $id;
    list($defaultvalues, $url) = literature_searchsource_sru_processform($data);
    ...
    // Set page data
    $PAGE->set_title(get_string('editsource', 'searchsource_sru'));
    $PAGE->set_heading($source->name);
} else {

    // Set page data
    $PAGE->set_title(get_string('editsource', 'searchsource_sru'));
    $PAGE->set_heading(get_string('newsource', 'searchsource_sru'));
}
...
$form = new literature_searchsource_sru_form('view.php?id=' . $id);
